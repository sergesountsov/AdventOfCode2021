...
var totalFish = fishCount.Aggregate(0,(count,next) => count + next);
Console.WriteLine ($"Part1: {totalFish}");

void PrintFish(int[] fishCount) {
    for  (int i =0; i < fishCount.Length; i++) {
        Console.WriteLine($"{i}: {fishCount[i]}");
    }
}

void Step(int[] fishCount) {
    int fishAtZero = fishCount[0];
